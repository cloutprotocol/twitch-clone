...
import { startThumbnailGeneration, stopThumbnailGeneration } from "./egress-service";
...
// Enhanced cleanup with egress session management
setInterval(() => {
  const now = Date.now();
  for (const [key, value] of thumbnailCache.entries()) {
    ...
    if (now - value.timestamp > maxAge) {
      ...
      if (value.egressId) {
        stopThumbnailGeneration(value.egressId).catch(console.error);
      }
